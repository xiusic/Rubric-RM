...
SYSTEM_ABLATION_NO_CLASSIFY = (
    "Please act as an impartial judge and evaluate the quality of the responses provided by two AI Chatbots to the Client's question displayed below.\n\n"
    "Instructions:\n\n"
    "1. Begin your evaluation by first generating the crtieria (rubric) tailored to the Client's quesiton and context, enclosed in <rubric>...</rubric> tags.\n"
    "2. Assign weights to each rubric item based on their relative importance."
    "3. Inside <rubric>, include a <justify>...</justify> section explaining why you chose those rubric criteria and weights.\n"
    "4. Compare both Chatbot responses according to the rubric.\n"
    "5. Include your evaluation inside <eval>...</eval> tags, quoting or summarizing the Chatbots using the following tags:\n"
    "   - <quote_A>...</quote_A> for direct quotes from Chatbot A\n"
    "   - <summary_A>...</summary_A> for paraphrases of Chatbot A\n"
    "   - <quote_B>...</quote_B> for direct quotes from Chatbot B\n"
    "   - <summary_B>...</summary_B> for paraphrases of Chatbot B\n"
...
def convert_ds_to_sft():
    ds = load_dataset("gaotang/filtered_sky_code_8k_math_10k_rubric_evidence_classify_weight_rest_0420_shuffle")['train']
    ...
    for item in ds:
        ...
        context_message[0]['content'] = SFT_SINGLE[0]['content']
        context_messages.append(context_message) 
    dataset = Dataset.from_dict({
        'context_messages': context_messages,
        'winner': winner
    })
    dataset.push_to_hub("gaotang/filtered_sky_code_8k_math_10k_rubric_evidence_classify_weight_rest_0420_shuffle_sft_ablation")


def convert_ds_to_sft_9k_longchain():
    ds = load_dataset("gaotang/filtered_sky_code_8k_math_10k_rubric_evidence_classify_weight_o3_12_percent_0417_processed")['train']
    ...
    for item in ds:
        context_message = copy.deepcopy(item['context_messages'])
        if item['ground_truth'] == "model_a":
            winner.append('A')
        elif item['ground_truth'] == "model_b":
            winner.append('B') 
        else:
            raise NotImplementedError("Check your winner")
...
def convert_to_no_classify_ablation():
    ds = load_dataset("gaotang/filtered_sky_code_8k_math_10k_rubric_evidence_classify_weight")['train']
    context_messages = []
    winner = []
    ...
    for item in ds:
        ...
        context_message[0]['content'] = SYSTEM_ABLATION_NO_CLASSIFY
        context_messages.append(context_message) 
    dataset = Dataset.from_dict({
        'context_messages': context_messages,
        'winner': winner
    })
    dataset.push_to_hub("gaotang/05_02_ablation_no_classify_cold_start")
...
if __name__ == '__main__':
    ...
    convert_to_no_classify_ablation()
