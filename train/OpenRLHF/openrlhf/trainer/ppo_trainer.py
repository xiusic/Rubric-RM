            ...
            for rand_prompts, labels in self.prompts_dataloader:
                ...
                for i, experience in enumerate(
                    ...
                    if args.verbose_training:
                        ...
                        outputs += output 
                        rewards += experience.info['reward'].detach().cpu().numpy().tolist()
                ...
                if args.verbose_training:
                    curr_output = [
                        ...
                        for (o, a, r) in zip(outputs, answers, rewards)
                    ]
                    log_dir = args.verbose_directory 
                    if log_dir is None:
                        log_dir = f"{args.save_path}/logs"
                    Path(log_dir).mkdir(parents=True, exist_ok=True)

                    with open(f"{log_dir}/Step_{steps}_training_logs.json", "w") as f:
                        json.dump(curr_output, f, indent=4)
